...
This tutorial will summarise details of the Joing Graphical Lasso (JGL) (Danaher, Wang, Witten 2011) algorithm, then walk through the steps needed to run the `JGL` package with metabolomics data provided by Raji Balasubramanian. 
...
* In our last seminar we learned details of graphical lasso to estimate precision matrix for Gaussian Graphical Models (GGMs). 
...
* There are two types of penalties we can use within JGL. 
...
* When $\lambda_2$ is **large**, many elements of $\hat{\Theta}^{(1)}, ... , \hat{\Theta}^{(K)}$ will be the same across classes  
* So, GGL just encourages a shared pattern of sparsity, not shared edge values (unlike FGL which encourages both)
...
library(pheatmap)
#source(here::here("code/helper_functions.R")) #contains code from Koyejo-lab/JointGraphicalLasso
...
From this heatmap we can see there are big differences in the values for metabolite group 3 (pink) across the 4 ancestry groups. metabolites from group 1 and group 2 appear to be more homogenous across the classes.
...
### Using Fused Graphical Lasso penalty
...
```{r visualize_fgl, fig.show="hold", out.width="50%"}
...
plot_jgl(graph_list[[1]], multiplier = 3, vertex.size = 6)
...
### Using Group Graphical Lasso penalty  

Now run "group" penalty instead. 
...
The Koyejo Lab  repository full of code to assist with different methods to select parameters for JGL. 

The below chunk is one example where they do a 2D grid search over a sequence of possible values for lambda1 and lambda2, collect Akaike Information Criterion (AIC) for each combination, then use the lambda combination with the lowest AIC for their final model with tuned parameters. 

You can use this method with both fused or group penalty. 

### Example tuning lambdas with metabolomics data

Below I'm making a very small 3x3 grid of lambdas for run-time demonstration purposes. 
...
### Run JGL again with tuned parameters
