...
Danaher et. al wrote the R package `JGL` as companion to their paper. As described above in Section 2.5, there are two penalty functions for `lambda2` described in the paper: **Fused Graphical Lasso (FGL)** and **Group Graphical Lasso (GGL)** penalties.

We will first go over some basic results for both penalties using preset values for lambda1 and lambda2 parameters, visualize the results, then discuss strategies for tuning the parameters.  
...
*Note: For now we are just putting in a pre-selected value for lambda1 and lambda2, but in a later section we will go over methods to tune the parameters.*
...
As we described above, having large lambda 1 forces sparse solutions for both penalty functions.

When lambda2 is high in FGL it enforces similar values across classes (aggressive sharing of information). 

When lambda2 is high in GGL it enforces similar sparsity locations across classes, but does not enforce similar values.
...
* Small lambda1 (0.01), small lambda2 (0.01)
* Small lambda1 (0.01), large lambda2 (0.75)
* Large lambda1 (0.5), small lambda2 (0.01)
* Large lambda1 (0.5), large lambda2 (0.75)
...
lowlow <- JGL(Y=dat_mat_list,penalty="group",lambda1=lambda.eff[1],lambda2=lambda.eff[1], return.whole.theta=TRUE)
test1 <- vBIC(dat_mat_list, lowlow, thr=0.0001)
paste("AIC for low lambda1 low lambda2:", round(test1, 2))
lowlowgraph <- graph_from_adjacency_matrix(
                    -cov2cor(lowlow$theta[[1]]),
                    weighted = T,
                    mode = "undirected",
                    diag = FALSE
                  )

lowhigh <- JGL(Y=dat_mat_list,penalty="group",lambda1=lambda.eff[1],lambda2=lambda.eff[2], return.whole.theta=TRUE)
test2 <- vBIC(dat_mat_list, lowhigh, thr=0.0001)
paste("AIC for low lambda1 high lambda2:", round(test2, 2))
lowhighgraph <- graph_from_adjacency_matrix(
                    -cov2cor(lowhigh$theta[[1]]),
                    weighted = T,
                    mode = "undirected",
                    diag = FALSE
                  )
...
highlow <- JGL(Y=dat_mat_list,penalty="group",lambda1=0.5,lambda2=lambda.eff[1], return.whole.theta=TRUE)
test3 <- vBIC(dat_mat_list, highlow, thr=0.0001)
paste("AIC for high lambda1 low lambda2:", round(test3, 2))
highlowgraph <- graph_from_adjacency_matrix(
                    -cov2cor(highlow$theta[[1]]),
                    weighted = T,
                    mode = "undirected",
                    diag = FALSE
                  )
...
highhigh <- JGL(Y=dat_mat_list,penalty="group",lambda1=0.4,lambda2=0.5, return.whole.theta=TRUE)
test4 <- vBIC(dat_mat_list, highhigh, thr=0.0001)
paste("AIC for high lambda1 high lambda2:", round(test4, 2))
highhighgraph <- graph_from_adjacency_matrix(
                    -cov2cor(highhigh$theta[[1]]),
                    weighted = T,
                    mode = "undirected",
                    diag = FALSE
                  )
...
#         fits[[length(fits + 1)]] <- fit00
#    }
# }
```

#### GGL Small lambda1 Small lambda2

```{r}
plot_jgl(lowlowgraph,
         vertex.color = color_list$color,
         vertex.label.color = color_list$color,
         multiplier = 3,
...
#### GGL Small lambda1 Large lambda2

```{r}
plot_jgl(lowhighgraph,
         vertex.color = color_list$color,
         vertex.label.color = color_list$color,
         multiplier = 3,
         main = "GGL small lambda1 large lambda2",
         sub = paste("AIC", round(test2, 2)))
```

#### GGL Large lambda1 Small lambda2
...
plot_jgl(lowhighgraph,
         ...
         sub = paste("AIC", round(test3, 2)))
```

#### GGL Large lambda1 Large lambda2

```{r}
plot_jgl(highhighgraph,
         vertex.color = color_list$color,
         vertex.label.color = color_list$color,
         multiplier = 3,
         main = "FGL large lambda1 large lambda2",
         sub = paste("AIC", round(test4, 2)))
```


```{r}

lowlow_f <- JGL(Y=dat_mat_list,penalty="fused",lambda1=lambda.eff[1],lambda2=lambda.eff[1], return.whole.theta=TRUE)
test1_f <- vBIC(dat_mat_list, lowlow_f, thr=0.0001)
lowlowgraph_f <- graph_from_adjacency_matrix(
                    -cov2cor(lowlow_f$theta[[1]]),
                    weighted = T,
                    mode = "undirected",
                    diag = FALSE
                  )

lowhigh_f <- JGL(Y=dat_mat_list,penalty="fused",lambda1=lambda.eff[1],lambda2=lambda.eff[2], return.whole.theta=TRUE)
test2_f <- vBIC(dat_mat_list, lowhigh_f, thr=0.0001)
lowhighgraph_f <- graph_from_adjacency_matrix(
                    -cov2cor(lowhigh_f$theta[[1]]),
                    weighted = T,
                    mode = "undirected",
                    diag = FALSE
                  )
...
highlow_f <- JGL(Y=dat_mat_list,penalty="fused",lambda1=0.5,lambda2=lambda.eff[1], return.whole.theta=TRUE)
test3_f <- vBIC(dat_mat_list, highlow_f, thr=0.0001)
highlowgraph_f <- graph_from_adjacency_matrix(
                    -cov2cor(highlow_f$theta[[1]]),
                    weighted = T,
                    mode = "undirected",
                    diag = FALSE
                  )
...
highhigh_f <- JGL(Y=dat_mat_list,penalty="fused",lambda1=0.4,lambda2=0.5, return.whole.theta=TRUE)
test4_f <- vBIC(dat_mat_list, highhigh_f, thr=0.0001)
highhighgraph_f <- graph_from_adjacency_matrix(
                    -cov2cor(highhigh_f$theta[[1]]),
                    weighted = T,
                    mode = "undirected",
                    diag = FALSE
                  )
...
#### FGL Small lambda1 Small lambda2

```{r}
plot_jgl(lowlowgraph_f,
         vertex.color = color_list$color,
         vertex.label.color = color_list$color,
         multiplier = 3,
         main = "FGL small lambda1 small lambda2",
         sub = paste("AIC", round(test1_f, 2)))
```

#### FGL Small lambda1 Large lambda2

```{r}
plot_jgl(lowhighgraph_f,
         vertex.color = color_list$color,
         vertex.label.color = color_list$color,
         multiplier = 3,
         main = "FGL small lambda1 large lambda2",
         sub = paste("AIC", round(test2_f, 2)))
```

#### FGL Large lambda1 Small lambda2

```{r}
plot_jgl(highlowgraph_f,
         vertex.color = color_list$color,
         vertex.label.color = color_list$color,
         multiplier = 3,
         main = "FGL large lambda1 small lambda2",
         sub = paste("AIC", round(test3_f, 2)))
```

#### FGL Large lambda1 Large lambda2

```{r}
plot_jgl(highhighgraph_f,
         vertex.color = color_list$color,
         vertex.label.color = color_list$color,
         multiplier = 3,
         main = "FGL large lambda1 large lambda2",
         sub = paste("AIC", round(test4_f, 2)))
```
...
Here I am demonstrating how to tune lambda1 by keeping lambda2 constant, then using that lambda1 to tune lambda2, as the authors suggest if doing 2D grid search is computationally prohibitive. 

```{r}
interval_l = 10
lambda.eff <- seq(0.01, 0.9, len = interval_l)
...
Here are the final tuned values for lambda1 and lambda2 using this method:
...
# test <- E(graph_list$ag1)
# test2 <- E(graph_list$ag2)
# 
# test_dif <- difference(graph_list$ag2, graph_list$ag1)
# E(test_dif)
# 
# test_intersect <- intersect(E(graph_list$ag1), E(graph_list$ag2))

test_edge1 <- as.data.frame(get.edgelist(graph_list$ag1))

test_edge2 <- as.data.frame(get.edgelist(graph_list$ag2))

test_edge3 <- as.data.frame(get.edgelist(graph_list$ag3))

test_edge4 <- as.data.frame(get.edgelist(graph_list$ag4))

shared_edges <- inner_join(test_edge1, test_edge2) %>%
  inner_join(test_edge3) %>%
  inner_join(test_edge4)

unique1 <- anti_join(test_edge1, test_edge2) %>%
  anti_join(test_edge3) %>%
  anti_join(test_edge4)

unique2 <- anti_join(test_edge2, test_edge1) %>%
  anti_join(test_edge3) %>%
  anti_join(test_edge4)

unique3 <- anti_join(test_edge3, test_edge1) %>%
  anti_join(test_edge2) %>%
  anti_join(test_edge4)

unique4 <- anti_join(test_edge4, test_edge1) %>%
  anti_join(test_edge2) %>%
  anti_join(test_edge3)
...
Site built using [GitHub pages](https://pages.github.com/) and RMarkdown. See [here](https://happygitwithr.com/rstudio-git-github.html) for Jenny Bryant's guide to using Git and Rmarkdown. 
