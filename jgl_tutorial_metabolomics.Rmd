...
```{r load_data_packages, message = FALSE, warning=FALSE, include=TRUE}
...
# specify colors for groups of metabolites 
...
Identify lambda combination with lowest AIC (visualized above). 
...
Here I am demonstrating how to tune lambda1 keeping lambda2 constant, then using that lambda1 to tune lambda2, as the authors suggest if doing 2D grid search is computationally prohibitive. 
...
What I take from these networks is that there appear to be dense edges within the metabolite classes, and fewer cross-class edges although there are a few. 
